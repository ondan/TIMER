...
io.on("connection", (socket) => {
    ...
    socket.on("join_room", ({ code, playerName }) => {
        ...
        if (!room) {
            emitError(socket.id, "Místnost neexistuje nebo hra již běží.");
            return;
        }
        socket.join(room.code);
    ...
    socket.on("set_game_mode", ({ mode }) => {
        ...
        if (!room || room.hostId !== socket.id) {
            emitError(socket.id, "Pouze host může změnit mód.");
            return;
        }
        if (!room.setGameMode(mode)) {
            emitError(socket.id, "Mód nelze změnit.");
            return;
        }
        broadcastRoom(room.code);
    });
    socket.on("start_game", () => {
        const room = roomManager.getRoomForPlayer(socket.id);
        if (!room || room.hostId !== socket.id) {
            emitError(socket.id, "Pouze host může spustit hru.");
            return;
        }
        if (!room.startGame()) {
            const min = room.modeInfo.minPlayers;
            emitError(socket.id, `Hru nelze spustit — mód ${room.modeInfo.name} potřebuje min. ${min} hráče.`);
            return;
        }
        broadcastRoom(room.code);
    });
    socket.on("timer_toggle", () => {
        ...
        if (!room.toggleTimer(socket.id, Date.now())) {
            emitError(socket.id, "Teď nemůžeš ovládat stopky.");
            return;
        }
        broadcastRoom(room.code);
    });
    socket.on("submit_guess", ({ guessMs }) => {
        ...
        if (!room.submitGuess(socket.id, guessMs)) {
            emitError(socket.id, "Odhad nelze odeslat.");
            return;
        }
        broadcastRoom(room.code);
    });
...
setInterval(() => {
    ...
    for (const roomCode of io.sockets.adapter.rooms.keys()) {
        ...
        if (!gameRoom?.hasRunningTimers())
            continue;
        if (gameRoom.usesPerPlayerTimers()) {
            const playerTimers = gameRoom.getRunningTimerSnapshot(now);
            if (Object.keys(playerTimers).length > 0) {
                io.to(roomCode).emit("timer_tick", { elapsedMs: 0, running: true, playerTimers });
            }
        }
        else if (gameRoom.timer.running) {
